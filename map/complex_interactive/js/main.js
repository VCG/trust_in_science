...
// d3.csv("data/data_fin.csv")

d3.csv("https://raw.githubusercontent.com/VCG/trust_in_science/barbara_branch/map/complex/data/data_fin.csv")
    ...
    states.forEach(state => {
        new LineChartSmallComplexInt(`.state-${state.code.toLowerCase()}`, data, state.code, maxNewCases);
        $(`.state-${state.code.toLowerCase()}`).click(() => {
            new LineChartLargeComplexInt("#chart-overlay", data, state.code);
